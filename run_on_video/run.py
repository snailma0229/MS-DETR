...
from ms_detr.span_utils import span_cxw_to_xx
from utils.basic_utils import l2_normalize_np_array
import torch.nn.functional as F
import numpy as np


class MSDETRPredictor:
    ...
    def localize_moment(self, video_path, query_list):
        ...
        assert n_frames <= 75, "The positional embedding of this pretrained MS_DETR only support video up " \
        ...
        # #moment_queries refers to the positional embeddings in MS_DETR's decoder, not the input text query
...
def run_example():
    ...
    ckpt_path = "run_on_video/ms_detr_ckpt/model_best.ckpt"

    # run predictions
    print("Build models...")
    clip_model_name_or_path = "ViT-B/32"
    # clip_model_name_or_path = "tmp/ViT-B-32.pt"
    ms_detr_predictor = MSDETRPredictor(
        ckpt_path=ckpt_path,
        clip_model_name_or_path=clip_model_name_or_path,
        device="cuda"
    )
    print("Run prediction...")
    predictions = ms_detr_predictor.localize_moment(
