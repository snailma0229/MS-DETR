...
from ms_detr.span_utils import span_xx_to_cxw
...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dir,
                 ...
                 dset_domain=None, mode='val', use_neg_captions=False):
        ...
        self.mode = mode
        self.use_neg_captions = use_neg_captions
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train" and self.use_neg_captions:
            model_inputs["query_feat_neg"] = self._get_query_neg_feat_by_qid(meta["qid"]) 
        if self.use_video:
    ...
    def _get_query_neg_feat_by_qid(self, qid):
        data_dir = self.q_feat_dir.replace("clip_text_features", "clip_text_features_neg_llama3")
        if isinstance(qid, str) and "_pos_llama3" in qid:
            qid = qid.replace("_pos_llama3","")
        if self.dset_name in ['tacos', 'nlq']:
            q_feat_path = join(data_dir, f"{qid}.npz")
        else:
            q_feat_path = join(data_dir, f"qid{qid}.npz")
        q_feat = np.load(q_feat_path)[self.q_feat_type].astype(np.float32)
        if self.q_feat_type == "last_hidden_state":
            q_feat = q_feat[:self.max_q_l]
        if self.normalize_t:
...
def prepare_batch_inputs(batched_model_inputs, device, use_neg_captions, non_blocking=False, mode='val'):
    if mode == 'train' and use_neg_captions:
        model_inputs = dict(
            src_txt=batched_model_inputs["query_feat"][0].to(device, non_blocking=non_blocking),
            src_txt_mask=batched_model_inputs["query_feat"][1].to(device, non_blocking=non_blocking),
            src_vid=batched_model_inputs["video_feat"][0].to(device, non_blocking=non_blocking),
            src_vid_mask=batched_model_inputs["video_feat"][1].to(device, non_blocking=non_blocking),
            src_txt_neg=batched_model_inputs["query_feat_neg"][0].to(device, non_blocking=non_blocking),
            src_txt_mask_neg=batched_model_inputs["query_feat_neg"][1].to(device, non_blocking=non_blocking),
            mode = mode,
        )
    else:
        model_inputs = dict(
