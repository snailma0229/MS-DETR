...
class Transformer_single(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
                 ...
                 activation="relu", normalize_before=False, num_patterns=0,
                 ):
        super().__init__()
    ...
    def forward(self, src, mask, pos_embed, video_length=None):
        ...
        return src[:video_length + 1], mask[:, :video_length + 1], pos_embed[:video_length + 1], src[video_length + 1:]
...
class Transformer(nn.Module):
    ...
    # for tvsum, add video_length in argument
    def forward(self, src_combine, mask, query_embed, input_query_label, input_query_bbox, attn_mask, opt, pos_embed, video_length=None):
        
       # flatten NxCxHxW to HWxNxC
        refpoint_embed = input_query_bbox

        bs, d= src_combine.shape[1], src_combine.shape[2]
        src_v = src_combine # (32, 75, 256)
        mask_v = mask[:, :video_length + 1] # (32, 75, 256)
        pos_embed_v = pos_embed

        memory = self.encoder(src_v, src_key_padding_mask=mask_v, pos=pos_embed_v)  # (L, batch_size, d)
        memory_global, memory_local = memory[0], memory[1:]
        mask_local = mask_v[:, 1:]
        pos_embed_local = pos_embed_v[1:]
        ...
        if opt.use_tgt:
            ...
            enc_outputs_coord_unselected = self.enc_out_span_embed(output_memory) + output_proposals # (bs, L, 2) unsigmoid
            topk = self.num_queries
            # topk_proposals = torch.topk(enc_outputs_class_unselected[..., 0], topk, dim=1)[1] # bs, nq
            
            if enc_outputs_class_unselected[..., 0].shape[1] >= topk:
                topk_proposals = torch.topk(enc_outputs_class_unselected[..., 0], topk, dim=1)[1] # (bs, nq) index
            else:
                num_repeats = topk // enc_outputs_class_unselected[..., 0].shape[1]
                remainder = topk % enc_outputs_class_unselected[..., 0].shape[1]
                topk_proposals = enc_outputs_class_unselected[..., 0].repeat(num_repeats, 1)
                if remainder > 0:
                    topk_proposals = torch.cat([topk_proposals, enc_outputs_class_unselected[..., 0][:, :remainder]], dim=1)  
                topk_proposals = torch.topk(topk_proposals, topk, dim=1)[1]  
        ...
        hs, references = self.decoder(tgt, memory_local, tgt_mask=attn_mask, memory_key_padding_mask=mask_local,
...
def build_transformer(args):
    if args.use_sf:
        ...
        from .transformer_normal import Transformer_normal
        return Transformer_normal(
            d_model=args.hidden_dim,
            dropout=args.dropout,
            nhead=args.nheads,
            dim_feedforward=args.dim_feedforward,
...
def build_transformer_single(args):
    return Transformer_single(
        d_model=args.hidden_dim,
        dropout=args.dropout,
        nhead=args.nheads,
        dim_feedforward=args.dim_feedforward,
        num_encoder_layers=args.cross_enc_layers,
        num_decoder_layers=args.dec_layers,
        normalize_before=args.pre_norm,
        activation='prelu',
    )
