...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument('--slowfast_dim', default=2306, type=int)
        parser.add_argument('--clip_dim', default=514, type=int)
 
        parser.add_argument('--use_dn', action="store_true",
                        help="Enable denoising training (noisy known-target queries + DN losses).")
        parser.add_argument('--use_dn_neg', action="store_true",
                        help="Add negative groups in DN (duplicate and mark negatives).")
        parser.add_argument('--use_tgt', action="store_true",
                        help="Initialize decoder queries from encoder proposals; enable encoder intermediate outputs.")
        parser.add_argument('--use_sf', action="store_true",
                            help='Use SlowFast+CLIP dual-branch pipeline.')
        parser.add_argument('--use_neg_captions', action="store_true",
                            help='Use mismatched (negative) captions during training.')
        parser.add_argument('--use_decoder_neg', action="store_true",
                            help='Decode and supervise negative pairs.')
        parser.add_argument('--interm_neg', action="store_true",
                        help="Add encoder intermediate supervision for negatives (with --use_tgt).")
        parser.add_argument('--use_interm', action="store_true",
                        help="Enable encoder intermediate losses (labels/span/GIoU).")

        parser.add_argument('--scalar', default=10, type=int,
                            help="Number of DN groups (reconstruction repetitions).")
        parser.add_argument('--label_noise_scale', default=0, type=float,
                            help="Ratio to flip DN labels (label noise).")
        parser.add_argument('--box_noise_scale', default=0.5, type=float,
                            help="Scale of DN box noise (shift/scale)") 
        parser.add_argument('--interm_loss_coef', default=1.0, type=float,
                            help="Loss coefficient for encoder intermediate supervision") 
        parser.add_argument('--num_patterns', default=0, type=int, 
                        help='Number of pattern embeddings (query replication).')
        parser.add_argument('--focal_alpha', type=float, default=0.25, 
                        help="Alpha parameter for focal loss")
        parser.add_argument('--cross_enc_layers', default=2, type=int,
                            help="Number of cross-encoder layers (Transformer_single)")
        ...
        parser.add_argument("--n_epoch", type=int, default=400, help="number of epochs to run")
    ...
    def parse(self, a_feat_dir=None):
        ...
        if isinstance(self, TestOptions):
            ...
            save_fns = ['ms_detr/model.py', 'ms_detr/transformer.py']
