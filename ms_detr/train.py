...
import sys
sys.path.append('xxx/MS-DETR')

from ms_detr.config import BaseOptions
from ms_detr.start_end_dataset_cg import StartEndDataset, start_end_collate, prepare_batch_inputs
from ms_detr.start_end_dataset_audio import StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from ms_detr.inference_cg import eval_epoch, start_inference, setup_model, get_dn_targets
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        if opt.a_feat_dir is None:
            ...
            model_inputs, targets = prepare_batch_inputs_audio(batch[1], opt.device, opt.use_neg_captions, non_blocking=opt.pin_memory, mode=mode)
        ...
        for i in loss_dict.keys():
            if 'error' not in i:
                assert i in criterion.weight_dict.keys()
