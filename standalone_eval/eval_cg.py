...
def compute_mr_ap(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10),
                  max_gt_windows=None, max_pred_windows=10, num_workers=8, chunksize=50, store=False):
    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    pred_qid2data = defaultdict(list) # 用于存储每个查询ID对应的预测结果
    for d in submission:
    ...
    if num_workers > 1: # 使用多进程计算AP
        ...
        for data_triple in data_triples:
            ...
            qid2ap_list[qid] = scores
    # 将AP结果转换为numpy数组，计算不同IoU阈值下的mAP
    ...
    if store:
        qid2ap_store = {}
        for qid, ap in qid2ap_list.items():
            qid2ap_store[qid] = float(ap.mean())
            import json
            with open("./store_ap.json", "w") as f:
                json.dump(qid2ap_store, f)
    ap_thds = ap_array.mean(0)  # mAP at different IoU thresholds.
    iou_thd2ap = dict(zip([str(e) for e in iou_thds], ap_thds))
    iou_thd2ap["average"] = np.mean(ap_thds) # 计算整体mAP
    # 格式化输出结果
...
def compute_mr_r1(submission, ground_truth, iou_thds=np.linspace(0.3, 0.95, 14)):
    ...
    # 准备预测窗口和真实窗口的numpy数组，用于批量计算IoU
    qids = list(pred_qid2window.keys()) # 获取所有查询ID
    pred_windows = np.array([pred_qid2window[k] for k in qids]).astype(float) # 预测窗口数组
    gt_windows = np.array([gt_qid2window[k] for k in qids]).astype(float) # 真实窗口数组
    pred_gt_iou = compute_temporal_iou_batch_paired(pred_windows, gt_windows)  # 计算预测窗口和真实窗口之间的IoU
    iou_thd2recall_at_one = {}  # 初始化存储不同IoU阈值下recall@1指标的字典
...
def eval_moment_retrieval(submission, ground_truth, verbose=True):
    ...
    for l_range, name in zip(length_ranges, range_names):
        ...
        if len(_ground_truth) == 0:
            ...
            iou_thd2average_precision = compute_mr_ap(_submission, _ground_truth, num_workers=8, chunksize=50, store=store)
