...
def prepare_for_cdn(dn_args, batch_size, mode, num_queries, num_classes, hidden_dim, label_enc, opt):
    ...
    if training and opt.use_dn:
        if opt.use_dn_neg:
            dn_number = dn_number # 这里应该是为了100的除法 特地乘了个2，——》 可以减免一下
        
        known = [(torch.ones_like(t['labels'])).cuda() for t in targets] # 全1向量 列表
        know_idx = [torch.nonzero(t) for t in known] # label个数的range列表
        known_num = [sum(k) for k in known] # label个数列表

        if int(max(known_num)) == 0: # 确定group
            dn_number = 1
        else:
            if dn_number >= 100:
        ...
        if opt.use_dn_neg:
            known_indice = known_indice.repeat(2 * dn_number, 1).view(-1) # known query所属的batch id
            known_labels = labels.repeat(2 * dn_number, 1).view(-1)
            known_bid = batch_idx.repeat(2 * dn_number, 1).view(-1) # known query所属的batch id
            known_bboxs = boxes.repeat(2 * dn_number, 1)
        else:
            known_indice = known_indice.repeat(dn_number, 1).view(-1) # known query所属的batch id
            known_labels = labels.repeat(dn_number, 1).view(-1)
            known_bid = batch_idx.repeat(dn_number, 1).view(-1) # known query所属的batch id
        ...
        if label_noise_ratio > 0:
            ...
            chosen_indice = torch.nonzero(p < (label_noise_ratio)).view(-1)  # usually half of bbox noise
            new_label = torch.randint_like(chosen_indice, 0, num_classes+1)  # randomly put a new one here
        ...
        if box_noise_scale > 0:
            ...
            known_bbox_[:, :1] = known_bboxs[:, :1] - known_bboxs[:, 1:] / 2 # (cx, cy, w, h)转换成(x,y,x,y)
            known_bbox_[:, 1:] = known_bboxs[:, :1] + known_bboxs[:, 1:] / 2

            diff = torch.zeros_like(known_bboxs) # 创建一个与known_bboxs形状相同但全为零的tensor，用于存放噪声范围
            diff[:, :1] = known_bboxs[:, 1:] / 2 # delta x1 和 delta y1 的范围的上限，即宽和高的一半
            diff[:, 1:] = known_bboxs[:, 1:] / 2 # delta x2 和 delta y2 的范围的上限，即宽和高的一半
            # 随机增或减的符号
            rand_sign = torch.randint_like(known_bboxs, low=0, high=2, dtype=torch.float32) * 2.0 - 1.0 # -1或者1的值
            rand_part = torch.rand_like(known_bboxs) # 生成一个与known_bboxs形状相同的随机数tensor
            if opt.use_dn_neg:
                rand_part[negative_idx] += 1.0 # 在negative_idx指定的索引位置上，随机数加1，确保在这些位置上的噪声更大
            rand_part *= rand_sign # 将随机数与随机符号相乘，得到最终的随机噪声值
            known_bbox_ = known_bbox_ + torch.mul(rand_part,  # 将噪声应用到边界框上，并乘以噪声比例因子
                                                  diff).cuda() * box_noise_scale
            known_bbox_ = known_bbox_.clamp(min=0.0, max=1.0) # 将边界框的值限制在0和1之间，可能是因为坐标值被归一化到了[0,1]的范围
            known_bbox_expand[:, :1] = (known_bbox_[:, :1] + known_bbox_[:, 1:]) / 2 # 转换成(cx, cy, w, h)
        ...
        if opt.use_dn_neg:
            ...
            known_labels[negative_idx] = 1 # 背景类
        else:
            for i in range(dn_number):
                if i == 0:
